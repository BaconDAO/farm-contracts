...
contract Farm is Ownable, AccessControl {
    ...
    function updateNFTBalance() internal {
        for (uint256 i = 0; i < NFTCount; i++) {
            ...
            uint256 currentNFTBalance = memberNFT.balanceOf(msg.sender);
            if (_balances[msg.sender] >= (cost)) {
                ...
                if (currentNFTBalance == 0) {
                    ...
                    memberNFT.mint(msg.sender);
                } else if (currentNFTBalance > 1) {
                    // have more than 1 right now, burn the extra
                    for (uint256 j = 1; j < currentNFTBalance; j++) {
                        uint256 idToBurn = memberNFT.tokenOfOwnerByIndex(
                            msg.sender,
                            j
                        );
                        memberNFT.burn(idToBurn);
                    }
                } else {
                ...
                // staked balance fallen below threshold, make sure we have 0 NFT
                if (currentNFTBalance > 0) {
                    // have more than 0 right now, burn the extra
                    for (uint256 j = 0; j < currentNFTBalance; j++) {
                        uint256 idToBurn = memberNFT.tokenOfOwnerByIndex(
                            msg.sender,
                            j
                        );
